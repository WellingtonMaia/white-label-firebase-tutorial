...
class ProductsFragment : Fragment() {
    ...
    private fun observeNavBackStack() {
        findNavController().run {
            val navBackStakeEntry = getBackStackEntry(R.id.productsFragment)
            val savedStateHandle = navBackStakeEntry.savedStateHandle
            ...
            navBackStakeEntry.lifecycle.addObserver(observer)
            viewLifecycleOwner.lifecycle.addObserver(LifecycleEventObserver { _, event ->
                if (event == Lifecycle.Event.ON_DESTROY) {
                    navBackStakeEntry.lifecycle.removeObserver(observer)
                }
            })
        }
    }
